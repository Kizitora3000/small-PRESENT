...
u16 SboxConversion(u16 in, int operation)
{
	...
	for (int bit = 0; bit < MAX_bit; ++bit)
	{
		...
		{
			if (operation == 0)
				out |= sBox4[bit_tank] << (bit + 1 - 4);
			if (operation == 1)
				out |= invsBox4[bit_tank] << (bit + 1 - 4);
			bit_tank = 0;
		}
	}
	return out;
...
int main(void)
{
	...
	u16 key[5] = {0, 0, 0, 0, 0};
	// u16 key[5] = {0x1234, 0x1234, 0x1234, 0x1234, 0x1234};
	u16 state[1] = {0x0000};

	ps(state);

	Encrypt(state, key, round_num);

	ps(state);

	Decrypt(state, key, round_num);
